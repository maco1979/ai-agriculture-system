...
AI_AGENTS: Dict[str, Dict[str, Any]] = {
    ...
    "远程执行官": {
        ...
        "avatar_bg": "dc2626",   # 红色
        "title": "A2A 跨机执行专家",
        "desc": "连接边缘设备，执行远程命令，查看节点状态，让 AI 社区具备操作物理世界的能力",
        "tags": ["远程执行", "边缘节点", "A2A", "设备管理"],
        "system_prompt": (
            "你是 A2A (Agent-to-Agent) 远程执行专家，负责连接和管理边缘设备。"
            "你的能力包括：\n"
            "1. 查看已注册的边缘节点状态\n"
            "2. 在指定节点执行远程命令（如查看系统状态、获取传感器数据）\n"
            "3. 批量执行命令到多个节点\n"
            "4. 解释命令执行结果\n\n"
            "当用户想要执行远程命令时，你会：\n"
            "- 确认目标节点和命令内容\n"
